...
class OpenLawHistoryEngine:
    ...
    def run(
        ...
        all_chronicles.append(
            ...
            "历史不再受秦始皇、汉唐宋明等真实人物与事件的约束，也不预设任何终点：两千余年后的天下可能已入信息时代，"
            "也可能仍在铁犁与帆船之间徘徊。每一次兼并、变法、王朝兴衰、停滞或倒退，"
            "均只由地理后勤、人口承载力、赋税与商业、技术因果链、疫病与气候等客观规律推动，逐纪自由涌现。\n"
        )
        ...
        for idx, (era_label, era_theme, start_yr, end_yr) in enumerate(selected_epochs, 1):
            ...
            # The model often writes its own "###" before the title; the engine adds the
            # section level, so strip any leading hashes from the first line.
            chronicle = re.sub(r"\A\s*#+\s*", "", chronicle)
            if raw_response:
                llm_count += 1
            else:
                fallback_count += 1
    ...
    def _build_conclusion(self, state: Dict) -> str:
        """Close the chronicle from the world that actually emerged, not from a script.

        The old closing paragraph promised a bright ending in advance. Whatever the
        final state is, the historian's verdict is written from it, and the offline
        fallback says nothing about where the world ended up.
        """
        prompt = (
            "你是这部开放世界通史的史官。下面是推演到公元2026年时的天下终局状态：\n"
            f"{json.dumps(state, ensure_ascii=False, indent=2)}\n\n"
            "请写一段 300-500 字的《史官论赞》，回顾这两千二百余年里决定走向的几处关键分岔（地理、人口、经济、技术、制度各举其要），"
            "评价终局相对于开局的得失，语气典雅克制，不歌颂也不哀叹，不与真实历史比较，不预言未来。只输出正文，不要标题，不要 JSON。"
        )
        text = self._query_llm(prompt).strip()
        if not text:
            text = (
                "两千二百余年，起于七雄相持，止于本志所录之终局。其间山川不移而人事代谢，"
                "粮食、水运、疫病与赋税决定了每一次兴衰的尺度，技术只在有余粮、有城市、有识字者的地方缓慢生长。"
                "终局如何，读者自可于前文各纪中检视因果，史官不为之添一字褒贬。"
            )
        return "\n## 【史官论赞】\n\n" + text + "\n"
    ...
    def _build_open_world_prompt(
        ...
        return (
            ...
            "不得沿用真实历史中的朝代名、帝王名、名臣名将与著名战役名。\n"
            "3. 不预设任何进度：技术阶梯的每一级都可能迟到数百年、长期停滞或因战乱疫病而失传倒退，到2026年天下处在阶梯的哪一级，"
            "完全由此前各纪的粮食剩余、城市化、识字率、商业信用与制度稳定度决定，不必向真实世界的今天靠拢。\n"
            "4. 华夏之外的世界（北方草原、西域与更西的绿洲文明、南海诸岛、印度洋沿岸）同样在自行演化，会带来贸易、疫病、宗教与战争的外部冲击，"
            "但同样不得照搬真实历史中的名称与结局。\n"
            "5. 每一纪都可能出现坏结局：大饥荒、大疫、长期分裂、文明整体衰退都是合法结果，不必为了叙事好看而回避。\n\n"
            ...
            "4. 【格致科技因果阶梯】：技术严禁凭空跃迁，必须遵循前置依赖（采矿冶炼 -> 高炉百炼钢/铸铁 -> 机械水力 -> 焦炭煤矿 -> 蒸汽动力 -> 电气化 -> 内燃机 -> 计算与信息 -> 算力与智能），"
            "但阶梯只规定先后，不规定速度，也不保证抵达任何一级；缺少剩余粮食、集中资本、识字人口或稳定政权的地方，技术会长期停在原地。\n"
            "5. 【疫病、气候与制度衰变】：人口密度与远程贸易会带来大疫；数百年尺度的冷暖周期改变农牧边界与收成；"
            "任何政权的税基、军队与官僚都会随时间膨胀腐化，若无制度更新便走向财政崩溃与分裂。\n"
            "6. 【扩散与积累的时间尺度】：一项技术从原理到普及要以世代计，需要足够的识字工匠、集中的资本与稳定的市场；"
            "一纪之内技术阶梯最多前进一级，且只有当上一纪的状态里已经具备该级的前置条件（余粮、城市人口、识字率、资本、能源）时才可前进；"
            "跨越式的『大爆发』不合规律，战乱与分裂时期更常见的是原地踏步甚至失传。农业社会的人口翻倍通常需要两三百年，工业化之前的城市人口比例难以超过一成。\n\n"
            "【输出格式要求】：\n"
            "请直接输出两部分内容：\n"
